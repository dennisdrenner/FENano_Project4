...
var resizePizzas = function(size) {
  ...
  //changed document.querySelector to getElementById which is faster
 ...
 function determineDx (elem, size) {
  ...
  function changePizzaSizes(size) {
    ...
    //took the two lines below out of the loop so as not to call layout every time.
    var dx = determineDx(document.getElementsByClassName("randomPizzaContainer")[0], size);
    var newwidth = (document.getElementsByClassName("randomPizzaContainer")[0].offsetWidth + dx) + 'px';

    //calculate container once outside of loop to avoid multiple DOM calls
    var container = document.getElementsByClassName("randomPizzaContainer")

//Changed for loop syntax so that len is only calculated once instead of at every pass through the loop
...
//moved pizzasDiv outside of the loop for greater efficiency
...
function updatePositions() {
  ...
  //changed from document.querySelector for greater efficiency
  var items = document.getElementsByClassName('mover');

 //calculate this value outside of the loop to avoid triggering layout continuously
  var scrollPosition = document.body.scrollTop;
  

 //declare phase outside of the loop 
  var phase;

 //change syntax to calculate items.length only once    
  for (var i = 0, len = items.length; i < len; i++) {
  phase = Math.sin(( scrollPosition/ 1250) + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //calculate number of pizzas to generate based on height of the user's screen
  var screenHeight = window.screen.height;
  var rows = (Math.ceil(screenHeight/100));
  var totalPizzas = rows * cols; 

  //declare elem and movingPizzas outside of loop 
  var elem; 
  ...
  for (var i = 0; i < totalPizzas; i++) {
    ...
    //changed from document.querySelector
